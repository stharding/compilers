...
# Class returned by code generation
class IRModule:
    def __init__(self, functions, imports):
        self.functions = functions
        self.imports = imports

# Encapsulation of a single function/import
class Function:
    def __init__(self, name, parameters, rettype):
        self.name = name
...
class GenerateCode(ast.NodeVisitor):
    ...
    def __init__(self):
        ...
        # List of functions created so far
    ...
    def visit_BoolLiteral(self, node):
        self.code.append(('CONSTI', int(node.value)))
    ...
    def visit_UnaryOp(self, node):
        ...
        elif node.op == '!' and node.operand.type == 'bool':
    ...
    def visit_PrintStatement(self, node):
        ...
        if node.value.type == 'int' or node.value.type == 'bool':
            code = 'PRINTI'
    ...
    def visit_FuncDeclaration(self, node):
        oldcode = self.code
        parms = [(p.name, p.type) for p in node.parameters]
        if node.body is not None:
            self.code = Function(node.name, parms, node.rettype.type)
            self.visit(node.body)
            self.functions.append(self.code)
            self.code = oldcode
        else:
            self.imports.append(Function(node.name, parms, node.rettype.type))
...
def compile_ircode(source):
    ...
    if not errors_reported():
        gen = GenerateCode()
        gen.visit(ast)
        return IRModule(gen.functions, gen.imports)
    else:
        return None
...
def main():
    ...
    if module:
        for func in module.imports:
            print(f'::: IMPORT: {func.name}({func.parameters})->{func.rettype}')

        for func in module.functions:
            print(":::")
            print(f"::: FUNCTION: {func.name}({func.parameters})->{func.rettype}")
            print(":::")

...
class CheckProgramVisitor(NodeVisitor):
    ...
    def __init__(self):
        ...
        # Combined local/global symbol table
        self.symbols = self.globals

        # Whether or not we're in a function
    ...
    def visit_ConstDeclaration(self, node):
        ...
        if node.name in self.locals:
            error(node.lineno, f'{node.name} redefined. Previous definition on {self.locals[node.name].lineno}')
        else:
    ...
    def visit_SimpleLocation(self, node):
        ...
        if node.usage == 'write' and not isinstance(sym, (VarDeclaration, Parameter)):
            error(node.lineno, f"Can't assign to {node.name}")
            node.type = None
        elif node.usage == 'read' and not isinstance(sym, (VarDeclaration,ConstDeclaration,Parameter)):
    ...
    def visit_IfStatement(self, node):
        ...
        if node.test.type != 'bool':
            error(node.lineno, f'Test must evaluate to a bool')
    ...
    def visit_FuncDeclaration(self, node):
        if self.in_function:
            error(node.lineno, f'Nested functions not supported')
            return
        self.visit(node.rettype)

        if lookup_type(node.name):
            error(node.lineno, f'{node.name} is the name of a type.')
            return
        ...
        self.visit(node.body)
        self.locals = self.symbols = self.globals
    ...
    def visit_ReturnStatement(self, node):
        ...
        if not self.in_function:
            error(node.lineno, 'return statement used outside a function')
        elif node.value.type != self.in_function.rettype.type:
            error(node.lineno, "return type doesn't match function type")
    ...
    def visit_FunctionCall(self, node):
        ...
        node.type = func.rettype.type
        
        if len(func.parameters) != len(node.args):
            error(node.lineno, f'{node.name} expected {len(func.parameters)} arguments')
            return
        
        for n, (parm, arg) in enumerate(zip(func.parameters, node.args), start=1):
            if parm.type != arg.type:
                error(node.lineno, f'Type mismatch in argument {n}. {parm.type} != {arg.type}')
